...
public class CellPanel extends JPanel {
    ...
    public CellPanel(int row, int column, ISudokuController controller) {
        ...
        for (int value = 1; value <= cellsPerEdge; value++) {
            candidate[value] = new CandidatePanel(row, column, value,
                    controller);
            add(candidate[value]);
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        } else if (!controller.isSet(row, column)
                && controller.isHighlighted(row, column)) {
            g.setColor(Constances.HIGHLIGHTED_CELL_BACKGROUND_COLOR);
        } else {
            g.setColor(Constances.CELL_BACKGROUND_COLOR);
        }
        g.fillRect(inset + 2, inset + 2, getWidth() - inset * 2 - correction,
                getHeight() - inset * 2 - correction);
        ...
        if (controller.isSet(row, column)) {
            paintValue(g, getWidth(), getHeight(),
                    controller.getValue(row, column));
        }
    }
    ...
    private void paintValue(Graphics g, int width, int height, int v) {
        ...
        g.drawString(
                value,
                (int) (1 + width / 2 - bounds.getWidth() / 2),
